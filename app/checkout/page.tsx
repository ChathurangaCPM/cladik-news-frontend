      ...
      interval = setInterval(() => {
        setProcessingStage((prev) => {
          if (prev >= loadStages.length - 1) {
            ...
            // Completed simulation! Write states and redirect
            localStorage.setItem("userPlan", planParam);
            localStorage.setItem("billingInterval", billingParam);
            localStorage.setItem("billingEmail", email || "developer@neuralpress.io");
    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#07090e] text-slate-800 dark:text-neutral-100 flex flex-col justify-between transition-colors duration-300 font-inter selection:bg-lime-400/30 relative overflow-hidden">
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between w-full border-b border-slate-200/50 dark:border-white/[0.05]">
        <div className="flex items-center gap-3">
          <Link href="/pricing" className="flex items-center gap-2 hover:opacity-80 transition">
            <ArrowLeft className="w-4 h-4 text-slate-400" />
            <Image src="/main-logo.png" width={32} height={32} className="w-7 h-7 object-contain" alt="NeuralPress" />
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 grid grid-cols-1 lg:grid-cols-12 gap-10 items-start w-full">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div>
            ...
            <p className="text-slate-500 dark:text-neutral-400 text-xs font-normal mt-2 leading-relaxed">
              Review your developer sandbox specifications. Payment methods are fully simulated—no actual money will be charged.
            </p>
          </div>

          {/* Plan Recap Card */}
          <div className="bg-white dark:bg-white/[0.01] border border-slate-200/80 dark:border-white/[0.05] rounded-3xl p-6 shadow-sm dark:shadow-none backdrop-blur-xl">
            <div className="flex justify-between items-start border-b border-slate-100 dark:border-white/[0.05] pb-4">
              <div>
                <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Selected Tier</span>
                <span className="text-lg font-extrabold text-slate-900 dark:text-white flex items-center gap-2 mt-0.5">
              ...
              <div className="text-right">
                ...
                <span className="text-xl font-black font-mono text-slate-900 dark:text-white mt-0.5">
                  ${monthlyPrice}
            ...
            <div className="py-4 border-b border-slate-100 dark:border-white/[0.05] space-y-3.5">
              <div className="flex justify-between text-xs font-medium">
                <span className="text-slate-500 dark:text-neutral-400">API Requests Allowance</span>
                <span className="text-slate-800 dark:text-white font-mono">
                  {planParam === "free" ? "100 / day (5 RPM Limit)" : planParam === "business" ? "100,000 / mo" : "1,000,000+ / mo"}
                </span>
              </div>
              <div className="flex justify-between text-xs font-medium">
                <span className="text-slate-500 dark:text-neutral-400">Bilingual translations</span>
                <span className="text-slate-800 dark:text-white">
                  {planParam === "free" ? "English basic only (5h old news)" : "Full English-Sinhala"}
                </span>
              </div>
              <div className="flex justify-between text-xs font-medium">
                <span className="text-slate-500 dark:text-neutral-400">Markdown format retrieval</span>
                <span className="text-slate-800 dark:text-white">
                  {planParam === "free" ? "Not Available" : "Available"}
                </span>
              </div>
              <div className="flex justify-between text-xs font-medium">
                <span className="text-slate-500 dark:text-neutral-400">Webhooks & Streaming</span>
                <span className="text-slate-800 dark:text-white font-mono">
                  {planParam === "free" ? "No real-time push" : planParam === "business" ? "Webhooks + Live SSE Stream" : "Webhooks + Real-Time SSE + Sockets"}
                </span>
              </div>
              <div className="flex justify-between text-xs font-medium">
                <span className="text-slate-500 dark:text-neutral-400">Topic & Sentiment Filters</span>
                <span className="text-slate-800 dark:text-white">
            ...
            <div className="pt-4 flex justify-between items-center text-sm font-bold text-slate-900 dark:text-white">
              <span>Today's Total Charges</span>
              <span className="text-2xl font-black font-mono text-indigo-600 dark:text-indigo-400">
                ${totalPrice}
                {isAnnual && <span className="text-[10px] font-normal text-slate-400 font-sans block text-right mt-0.5">Billed Annually</span>}
              </span>
            </div>
          </div>
          ...
          <div className="bg-emerald-500/[0.03] border border-emerald-500/10 rounded-2xl p-4 flex gap-3.5 items-start">
            ...
            <div>
              <h5 className="text-xs font-bold text-emerald-700 dark:text-emerald-400">Simulated Sandbox Environment</h5>
              <p className="text-[11px] text-emerald-600/80 dark:text-emerald-400/60 leading-normal mt-0.5 font-light">
                NeuralPress Sandbox simulates real subscription integrations. Fill out card fields with any fake data to immediately experience how the dashboard dynamically adapts features.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-white dark:bg-white/[0.01] border border-slate-200/80 dark:border-white/[0.05] rounded-3xl p-6 sm:p-8 shadow-sm dark:shadow-none backdrop-blur-xl space-y-8 relative">
              ...
              >
                ...
                <div className="absolute w-full h-full backface-hidden p-6 flex flex-col justify-between z-10">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="text-[9px] uppercase tracking-widest text-slate-400 font-mono font-medium block">
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      <span className="text-[8px] text-slate-400 uppercase font-mono font-medium block">
                        Cardholder Name
                      </span>
                    ...
                    <div>
                      <span className="text-[8px] text-slate-400 uppercase font-mono font-medium block text-right">
                        Expires
                      </span>
                ...
                <div className="absolute w-full h-full backface-hidden rotateY-180 p-6 flex flex-col justify-between z-10">
                  ...
                  <div className="my-8 flex justify-end items-center gap-3">
                    <span className="text-[8px] text-slate-400 font-mono font-medium">SIGNATURE</span>
          ...
          <form onSubmit={handleCheckoutSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              className="w-full py-4 mt-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold shadow-md shadow-indigo-600/10 hover:scale-[1.01] active:scale-[0.99] transition duration-200 flex items-center justify-center gap-2"
            >
              {planParam === "free" ? "Activate Sandbox For Free" : `Activate ${planTitles[planParam]} • $${totalPrice}`}
          ...
          >
            ...
            >
              ...
              <div className="space-y-2">
                <h4 className="text-lg font-bold text-white tracking-tight">Activating API Credentials</h4>
                <p className="text-xs text-slate-400 font-normal">
                  Your sandbox credentials are being securely generated and allocated.
                </p>
              </div>
