...
export const ReportButton = ({ id, className }: { id: string; className?: string }) => {
  ...
  useEffect(() => {
    if (id && typeof window !== 'undefined') {
      try {
        const reported = JSON.parse(localStorage.getItem('reportedNews') || '[]');
  ...
  const buttonContent = (
    <button
      ...
      className={cn(
        "transition-all p-2 rounded-full border border-transparent outline-none",
        isReported
    ...
    >
      <Flag
        className={cn("w-[16px] h-[16px]", isReported ? "fill-red-500" : "")}
      />
    </button>
  );
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      <AlertDialogTrigger asChild>
        <div 
          className="inline-flex items-center space-x-1 font-inter" 
      ...
      <AlertDialogContent className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl font-inter text-left">
        <AlertDialogHeader className="text-left gap-1.5 p-0">
          ...
          <AlertDialogDescription className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            This will submit a report to our moderation team indicating that this news article is inappropriate or violates our policies.
          </AlertDialogDescription>
        </AlertDialogHeader>
