...
export default function Hero() {
  ...
  const getHeroCardData = (index: number) => {
    ...
    if (article) {
      const category = article.categories?.[0] || article.originalSource || (isSinhala ? "නවතම පුවත්" : "LATEST NEWS");
      const title = isSinhala ? (article.sinhalaTitle || article.title) : article.title;
      ...
      return {
        ...
        isDynamic: true,
        path,
        tagColor: index === 0 ? "text-[#2b86ff]" : index === 1 ? "text-lime-600" : index === 2 ? "text-emerald-500" : index === 3 ? "text-purple-500" : "text-amber-500",
        iconBg: index === 0 ? "bg-blue-50 text-[#2b86ff]" : index === 1 ? "bg-lime-50 text-lime-600" : index === 2 ? "bg-emerald-50 text-emerald-500" : index === 3 ? "bg-purple-50 text-purple-500" : "bg-amber-50 text-amber-500"
      };
    }
    ...
    const fallbacks = [
      {
        ...
        text: isSinhala ? "ගෝලීය පුවත් ක්ෂණිකව සිංහල භාෂාවෙන්." : "Instant translation to premium Sinhala.",
      ...
      },
      {
        ...
        text: isSinhala ? "අපගේ තාක්ෂණය නිරන්තරයෙන් පුවත් දර්ශකගත කරයි." : "Our engine continuously indexes search maps.",
      ...
      },
      {
        ...
        text: isSinhala ? "ගෝලීය පුවත් ප්‍රභවයන් 14ක් එකඟතාවයකට ගෙන ඇත." : "Consolidated 14 global story duplicate signals.",
      ...
      },
      {
        ...
        text: isSinhala ? "පුවත් සංකල්ප සිතියම්කරණය සක්‍රීයයි..." : "Tracing concept concept mappings...",
      ...
      },
      {
        ...
        text: isSinhala ? "සත්‍යාපනය කරන ලද සක්‍රීය පුවත් වාර්තා." : "Consensus-verified active story.",
  ...
  return (
    <div className="relative bg-gradient-to-b from-[#2b86ff] via-[#489cff] to-[#fafbfe] pt-6 pb-28 overflow-hidden text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mt-16 md:mt-24 relative z-10 md:pt-[40px]">
        <h1 className="text-5xl md:text-5xl tracking-tight leading-[1.05] max-w-5xl mx-auto text-white">
          Bridging global news with conceptual clarity<br />
          <span className="font-heading italic font-extralight">and bilingual AI.</span>
        </h1>
        <p className="mt-8 text-base text-white/85 font-light max-w-2xl mx-auto leading-relaxed">
          Discover a smarter, conceptual way to navigate global news. NeuralPress aggregates
          multiple perspectives, translates articles seamlessly, and ensures complete clarity.
        </p>
        ...
        <div className="mt-20 max-w-5xl mx-auto relative flex items-center justify-center h-[220px] select-none">
          {/* Card 1 */}
          <Link href={getHeroCardData(0).path} className="absolute left-[2%] z-10 block">
            <motion.div
            ...
            >
              <span
                className={`text-[10px] font-semibold ${getHeroCardData(0).tagColor
              ...
              <p
                className={`text-xs font-bold text-slate-800 leading-snug line-clamp-3 ${isSinhalaText(getHeroCardData(0).text)
              ...
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center ${getHeroCardData(0).iconBg
                  }`}
              >
          ...
          {/* Card 2 */}
          <Link href={getHeroCardData(1).path} className="absolute left-[22%] z-10 block">
            <motion.div
            ...
            >
              <span
                className={`text-[10px] font-semibold ${getHeroCardData(1).tagColor
              ...
              <p
                className={`text-xs font-bold text-slate-800 leading-snug line-clamp-3 ${isSinhalaText(getHeroCardData(1).text)
                  ? "font-sinhala tracking-normal text-sm font-medium leading-relaxed"
                  : ""
                  }`}
              >
              ...
              {getHeroCardData(1).isDynamic ? (
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center ${getHeroCardData(1).iconBg
                    }`}
                >
          ...
          <Link href={getHeroCardData(2).path} className="absolute z-20 block">
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                <span
                  className={`text-[10px] font-bold uppercase ${getHeroCardData(2).tagColor
              ...
              <p
                className={`text-xs font-extrabold text-slate-800 leading-normal line-clamp-3 ${isSinhalaText(getHeroCardData(2).text)
                  ? "font-sinhala tracking-normal text-sm font-medium leading-relaxed"
                  : ""
                  }`}
              >
          ...
          {/* Card 4 */}
          <Link href={getHeroCardData(3).path} className="absolute right-[22%] z-10 block">
            <motion.div
              initial={{ x: -8, y: 8, rotate: 6 }}
              animate={{ y: [8, -2, 8] }}
              whileHover={{ scale: 1.05, y: -12, rotate: 2, zIndex: 50 }}
            ...
            >
              <span
                className={`text-[10px] font-semibold ${getHeroCardData(3).tagColor
              ...
              <p
                className={`text-xs font-bold text-slate-800 leading-snug line-clamp-3 ${isSinhalaText(getHeroCardData(3).text)
                  ? "font-sinhala tracking-normal text-sm font-medium leading-relaxed"
                  : ""
                  }`}
              >
              ...
              {getHeroCardData(3).isDynamic ? (
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center ${getHeroCardData(3).iconBg
                    }`}
                >
          ...
          {/* Card 5 */}
          <Link href={getHeroCardData(4).path} className="absolute right-[2%] z-10 block">
            <motion.div
              initial={{ x: -16, y: 32, rotate: 12 }}
              animate={{ y: [32, 22, 32] }}
              whileHover={{ scale: 1.05, y: 12, rotate: 6, zIndex: 50 }}
            ...
            >
              <span
                className={`text-[10px] font-semibold ${getHeroCardData(4).tagColor
              ...
              <p
                className={`text-xs font-bold text-slate-800 leading-snug line-clamp-3 ${isSinhalaText(getHeroCardData(4).text)
                  ? "font-sinhala tracking-normal text-sm font-medium leading-relaxed"
                  : ""
                  }`}
              >
              ...
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center ${getHeroCardData(4).iconBg
                  }`}
              >
